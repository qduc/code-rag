    ...
    return [
        Tool(
            ...
            description=(
                ...
                "\n"
                "  1. **BEST: Specific identifiers + behavior context**\n"
                "     • 'StreamingNotSupportedError retry logic' → finds exact error handling\n"
                "     • 'persistence.markError cleanup' → finds all error state management\n"
                "     • 'handleRegularStreaming client disconnect' → finds specific handler\n"
                "\n"
                "  2. **GOOD: Natural language with specific concepts**\n"
                "     • 'upstream stream fails client disconnect what happens' → finds error flows\n"
                "     • 'tool orchestration error recovery persistence' → finds error handling patterns\n"
                "     • 'authentication token refresh queue requests' → finds auth logic\n"
                "\n"
                "  3. **EXPLORATORY: Behavior-based (when you don't know names)**\n"
                "     • 'code that validates API keys before requests'\n"
                "     • 'retry logic for failed network requests'\n"
                "     • 'caching strategy to avoid expensive operations'\n"
                "\n"
                "  4. **AVOID: Exact code syntax or generic terms**\n"
                "     ❌ 'req.on(\"close\", () => {' → semantic search doesn't understand syntax\n"
                "     ❌ 'error handling' → too broad, use specific error types\n"
                "     ❌ 'function that does X' → omit filler words, just describe X\n"
                "\n"
                "  **Tips:**\n"
                "  - If you know class/function/method names from the codebase, USE THEM in queries\n"
                "  - Combine specific identifiers with what you want to learn: \"ClassName behavior\"\n"
