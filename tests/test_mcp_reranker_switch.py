...
async def test_api_search_rerank_parameter():
    ...
    with (
        ...
        # Setup mock config to avoid shared server issues
        ...
        # Mock the reranker
        api.reranker = MagicMock()
        api.reranker.rerank.return_value = [(0, 0.95)]
        ...
        # Mock active collection
        api._active_collection = "test"

        # Call search with rerank=True
        api.search("query", rerank=True)
        assert api.reranker.rerank.called

        # Call search with rerank=False
        api.reranker.rerank.reset_mock()
        api.search("query", rerank=False)
        assert not api.reranker.rerank.called
